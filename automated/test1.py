...
with open('vtoutput-url.json', 'w+') as f:
                f.write(response.text)
command = 'grep "id" vtoutput-url.json | cut -d ":" -f 2'

cut_result = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
print(cut_result.stdout)
url = "https://www.virustotal.com/api/v3/analyses/"+cut_result.stdout
